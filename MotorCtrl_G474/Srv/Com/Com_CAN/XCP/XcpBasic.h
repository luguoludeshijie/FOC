...
/******************************************************************************/
  ...
  #define CRM_BYTE(x)             (xcp.Crm.b[x])
    ...
    #define CRO_WORD(x) \
        (((vuint16)pCmd->b[((x)*2)+1] << 8) | ((vuint8)pCmd->b[((x)*2)+0]))
    #define CRO_DWORD(x) \
        (((vuint32)pCmd->b[((x)*4)+3] << 24) | ((vuint32)pCmd->b[((x)*4)+2] << 16)| \
        ((vuint16)pCmd->b[((x)*4)+1] << 8)| ((vuint8)pCmd->b[((x)*4)+0]))

    #define CRM_WORD(x) \
        (((vuint16)xcp.Crm.b[((x)*2)+1] << 8) | ((vuint8)xcp.Crm.b[((x)*2)+0]))
    #define CRM_WORD_WRITE(x, d) \
        {xcp.Crm.b[((x)*2)+0] = ((vuint8)(d) & 0xFF); xcp.Crm.b[((x)*2)+1] = \
        ((vuint8)((vuint16)(d) >> 8) & 0xFF);}
    #define CRM_DWORD(x) \
        (((vuint32)xcp.Crm.b[((x)*4)+3] << 24) | ((vuint32)xcp.Crm.b[((x)*4)+2] << 16) | \
        ((vuint16)xcp.Crm.b[((x)*4)+1] << 8) | ((vuint8)xcp.Crm.b[((x)*4)+0]) )
    #define CRM_DWORD_WRITE(x, d) \
        {xcp.Crm.b[((x)*4)+0] = ((vuint8)((vuint32)(d) >> 0) & 0xFF); \
        xcp.Crm.b[((x)*4)+1] = ((vuint8)((vuint32)(d) >> 8) & 0xFF); \
        xcp.Crm.b[((x)*4)+2] = ((vuint8)((vuint32)(d) >> 16) & 0xFF); \
        xcp.Crm.b[((x)*4)+3] = ((vuint8)((vuint32)(d) >> 24) & 0xFF);}
    #else
    #define CRO_WORD(x) \
        (((vuint16)pCmd->b[((x)*2)+0] << 8) | ((vuint8)pCmd->b[((x)*2)+1]))
    #define CRO_DWORD(x) \
        (((vuint32)pCmd->b[((x)*4)+0] << 24) | ((vuint32)pCmd->b[((x)*4)+1] << 16)| \
        ((vuint16)pCmd->b[((x)*4)+2] << 8)| ((vuint8)pCmd->b[((x)*4)+3]))

    #define CRM_WORD(x) \
        (((vuint16)xcp.Crm.b[((x)*2)+0] << 8) | ((vuint8)xcp.Crm.b[((x)*2)+1]))
    #define CRM_WORD_WRITE(x, d) \
        {xcp.Crm.b[((x)*2)+1] = ((vuint8)(d) & 0xFF); xcp.Crm.b[((x)*2)+0] = \
        ((vuint8)((vuint16)(d) >> 8) & 0xFF);}
    #define CRM_DWORD(x) \
        (((vuint32)xcp.Crm.b[((x)*4)+0] << 24) | ((vuint32)xcp.Crm.b[((x)*4)+1] << 16) | \
        ((vuint16)xcp.Crm.b[((x)*4)+2] << 8) | ((vuint8)xcp.Crm.b[((x)*4)+3]) )
    #define CRM_DWORD_WRITE(x, d) \
        {xcp.Crm.b[((x)*4)+3] = ((vuint8)((vuint32)(d) >> 0) & 0xFF); \
...
typedef vuint16 SessionStatusType;
        ...
        #else
            #error "kXcpDaqTimestampSize not defined. Please define a \
  ...
  /* j is absolute odt number */
    #define DaqListOdtEntryCount(j) \
        ((xcp.pOdt[j].lastOdtEntry-xcp.pOdt[j].firstOdtEntry)+1)
    ...
    /* i is daq number */
    #define DaqListOdtCount(i) \
        ((xcp.Daq.u.DaqList[i].lastOdt-xcp.Daq.u.DaqList[i].firstOdt)+1)
...
/******************************************************************************/
    ...
    typedef vuint16 tXcpChecksumSumType;   
    /* Data type (width) of the actual checksum. */
    #endif
    ...
    typedef vuint32 tXcpChecksumSumType;   
    /* Data type (width) of the actual checksum. */
    #endif
#endif
...
#define XCP_SEND_PENDING \
    (XCP_DTO_PENDING|XCP_CRM_PENDING|XCP_EVT_PENDING)
...
#if defined ( XCP_ENABLE_CALIBRATION_MEM_ACCESS_BY_APPL ) && \
    !defined ( XCP_ENABLE_MEM_ACCESS_BY_APPL )
extern vuint8 ApplXcpCalibrationWrite(MTABYTEPTR addr, vuint8 size, \
    const BYTEPTR data);
extern vuint8 ApplXcpCalibrationRead(MTABYTEPTR addr, vuint8 size, \
    BYTEPTR data);
#endif
...
/* Used only, if read protection of memory areas is required */
    ...
    #else
extern vuint8 ApplXcpCalculateChecksum( ROMBYTEPTR pMemArea, BYTEPTR pRes, \
    vuint32 length );
    #endif
#endif
...
extern vuint8 ApplXcpCopyCalPage( vuint8 srcSeg, vuint8 srcPage, \
...
/* Check range of kXcpStimOdtCount */
  ...
  /* Check XCP_ENABLE_ODT_SIZE_WORD in combination with XCP_ENABLE_DAQ_HDR_ODT_DAQ */
    ...
    #if defined ( XCP_ENABLE_DAQ_HDR_ODT_DAQ )
    #else
      #error "XCP error: If XCP_ENABLE_ODT_SIZE_WORD is enabled, \
        XCP_ENABLE_DAQ_HDR_ODT_DAQ must be enabled as well."
    #endif
  #endif
  ...
  /* Check configuration of kXcpDaqTimestampTicksPerUnit. */
        ...
        #error "XCP error: illegal range of kXcpDaqTimestampTicksPerUnit: \
            0 < kXcpDaqTimestampTicksPerUnit <= 0xFFFF."
    #endif
  #endif

  /* Check for configuration of kXcpDaqTimestampSize */
  #if defined ( kXcpDaqTimestampSize )
    #if ( kXcpDaqTimestampSize != DAQ_TIMESTAMP_BYTE ) && ( kXcpDaqTimestampSize != \
        DAQ_TIMESTAMP_WORD ) && ( kXcpDaqTimestampSize != DAQ_TIMESTAMP_DWORD )
        #error "XCP error: Please define kXcpDaqTimestampSize to either \
            DAQ_TIMESTAMP_BYTE, DAQ_TIMESTAMP_WORD or DAQ_TIMESTAMP_DWORD"
    #endif
  #endif
