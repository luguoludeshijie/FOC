...
typedef struct __UDS_SERVICE_T__
{
    ...
    Boolean (*check_len)(const uint8_t *, uint16_t); // 检查数据长度是否合法
    Boolean std_spt;                                 // 是否支持默认会话
    Boolean prog_spt;                                // 是否支持编程会话
    Boolean ext_spt;                                 // 是否支持扩展会话
    Boolean fun_spt;                                 // 是否支持功能寻址
    Boolean ssp_spt;                                 // 是否支持肯定响应抑制
