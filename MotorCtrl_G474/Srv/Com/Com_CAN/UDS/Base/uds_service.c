...
void uds_negative_rsp(uint8_t sid, uds_nrc_em rsp_nrc)
{
    ...
    {
        if (NRC_SERVICE_NOT_SUPPORTED == rsp_nrc || NRC_SUBFUNCTION_NOT_SUPPORTED == rsp_nrc || NRC_REQUEST_OUT_OF_RANGE == rsp_nrc || NRC_SUBFUNCTION_NOT_SUPPORTED_IN_ACTIVE_SESSION == rsp_nrc || NRC_SERVICE_NOT_SUPPORTED_IN_ACTIVE_SESSION == rsp_nrc)
            return;
    }
...
static void uds_data_indication(uint8_t *msg_buf, uint16_t msg_dlc, n_result_t n_result)
{
    ...
    for (i = 0; i < SID_NUM; i++)
    {
        ...
        // 检查会话状态是否支持
        if ((UDS_SESSION_STD == get_current_session() && FALSE == uds_service_list[i].std_spt) || (UDS_SESSION_PROG == get_current_session() && FALSE == uds_service_list[i].prog_spt) || (UDS_SESSION_EXT == get_current_session() && FALSE == uds_service_list[i].ext_spt))
        {
